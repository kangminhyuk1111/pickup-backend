...
class MemberTest {
    ...
    void 생성자로_멤버_생성_테스트시_잘못된_값_입력() {
        final AddMemberRequest validFailByEmailEmptyRequest = new AddMemberRequest("", "encodedPassword", "농구왕", 180, 75, Position.PG, Level.BEGINNER);
        assertThrows(ValidateException.class, () -> validFailByEmailEmptyRequest.toEntity(passwordEncoder));

        final AddMemberRequest validFailByPasswordEmptyRequest = new AddMemberRequest("example@email.com", "", "농구왕", 180, 75, Position.PG, Level.BEGINNER);
        assertThrows(ValidateException.class, () -> validFailByPasswordEmptyRequest.toEntity(passwordEncoder));

        final AddMemberRequest validFailByNicknameEmptyRequest = new AddMemberRequest("example@email.com", "encodedPassword", "", 180, 75, Position.PG, Level.BEGINNER);
